...
class DemoApi(val context: Context) : InterviewdApiService {
    ...
    override fun deleteQuestion(id: Int): Single<QuestionDto> {
        val reader = context.openFileInput(questionsFilename)
        var json = reader.bufferedReader().readText()
        reader.close()
    ...
    override fun createTemplate(template: TemplateDto): Single<TemplateDto> {
        ...
        val mutableList = list.toMutableList()
        val toAdd = TemplateDto(template.name, template.questionIds, mutableList.size + 1)
        mutableList.add(toAdd)
        list = mutableList.toList()
    ...
    override fun deleteTemplate(id: Int): Single<TemplateDto> {
        val reader = context.openFileInput(templatesFilename)
        var json = reader.bufferedReader().readText()
        reader.close()
    ...
    override fun patchTemplate(id: Int, patch: TemplateDto): Single<TemplateDto> {
        val reader = context.openFileInput(templatesFilename)
        var json = reader.bufferedReader().readText()
        reader.close()
    ...
    override fun deleteInterview(id: Int): Single<InterviewDto> {
        val reader = context.openFileInput(interviewsFilename)
        var json = reader.bufferedReader().readText()
        reader.close()
